        ...
        <div class="  border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="flex items-center  h-48  rounded  dark:bg-gray-800">
                    <p class="text-4xl text-gray-900 dark:text-white">Halaman Editor</p>
                </div>
            </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <div class="flex justify-end mb-2 p-4  md:w-auto md:flex-row md:items-start">
                    <button type="button"
                        ...
                        <a href="{{ route('admin.tambahEditor') }}">Tambah</a>
                    </button>
                </div>
                @if (session('pesan'))
                    ...
                    <tbody>
                        ...
                        @foreach ($editor as $data)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">{{ $data->id_editor }}</td>
                                <td class="px-6 py-4">{{ $data->nama_editor }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.editEditor', $data->id_editor) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                    <form action="{{ route('admin.deleteEditor', $data->id_editor) }}" method="post">
                                        @csrf
                                        @method('DELETE')
